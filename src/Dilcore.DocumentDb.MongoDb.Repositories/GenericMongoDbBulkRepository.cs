...
public class GenericMongoDbBulkRepository<TDocument>(
...
{
    public Task<Result<IReadOnlyList<TDocument>>> BulkStoreAsync(TDocument[] entities,
    ...
    private static async Task<Result> SoftDeleteManyAsync(IMongoCollection<TDocument> collection,
        FilterDefinition<TDocument> filter, CancellationToken token)
    {
    ...
    private static async Task<Result> PermanentDeleteAsync(IMongoCollection<TDocument> collection,
        FilterDefinition<TDocument> filter, CancellationToken token)
    {
